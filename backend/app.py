...
# Enable CORS for all routes (needed when embedded in iframe)
CORS(app)
...
def get_agents():
    ...
    # Auto-refresh dandan status so it never goes offline
    for agent in agents:
        if agent.get("agentId") == "dandan":
            agent["lastPushAt"] = now.isoformat()
    ...
    for a in agents:
        ...
        # 2) 超时未推送自动离线（超过5分钟）
        #    但 dandan 是本地常驻agent，不自动离线
        last_push_at_str = a.get("lastPushAt")
        if auth_status == "approved" and last_push_at_str and a.get("agentId") != "dandan":
            try:
...
@app.route("/local-agent-update", methods=["POST"])
def local_agent_update():
    """Local endpoint to update dandan agent state without auth.
    
    Used by cron jobs to keep dandan online.
    """
    try:
        data = request.get_json() or {}
        state = data.get("state", "idle")
        detail = data.get("detail", "待命")
        
        agents = load_agents_state()
        for agent in agents:
            if agent.get("agentId") == "dandan":
                agent["state"] = normalize_agent_state(state)
                agent["detail"] = detail
...
def get_memos():
    """Get memos (alias for yesterday-memo for frontend compatibility)"""
    try:
        target_date = get_yesterday_date_str()
        MEMORY_DIR = os.path.join(os.path.dirname(ROOT_DIR), "memory")
        target_file = os.path.join(MEMORY_DIR, f"{target_date}.md")
